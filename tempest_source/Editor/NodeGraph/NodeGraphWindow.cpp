...
Editor::nodeGraphWindow::nodeGraphWindow(editorWindow * p_parent_editor, engineController * p_engine_controller)
  : windowBase(p_parent_editor), m_editor_window(p_parent_editor), m_draw(true), m_show_grid(true), m_open_context_menu(false),
  m_first_run(false), m_node_manager(new NodeManager(p_engine_controller, getLogger())),
  m_node_hovered_in_list(-1), m_node_hovered_in_scene(-1), m_node_slot_radius(4.0f), m_node_window_padding(8.0f, 8.0f),
  m_zoom_scalar(1.0f), m_graph_name("Unnamed_Graph"), m_open_graph(false), m_dirty_graph(false),
  m_connect_node_line_start(0.0f, 0.0f), m_output_link_node(std::pair<int, int>(-1, -1)),m_connect_node_line_end(0.0f, 0.0f), m_drawing_connect_line(false)
{
  ...
  for (auto i_node = l_node_list->begin(); i_node != l_node_list->end(); ++i_node)
  {
    ...
    for (int slot_idx = 0; slot_idx < l_node->getInputs(); slot_idx++)
    {
      ...
      if (ImGui::IsItemHovered(ImGuiHoveredFlags_AllowWhenBlockedByActiveItem))
      {
        ...
        //Left Alt + Left click
        else if ((ImGui::IsKeyDown(GLFW_KEY_LEFT_ALT) && ImGui::IsMouseClicked(0)))
        {
          //Decouple this node
    ...
    for (int slot_idx = 0; slot_idx < l_node->getOutputs(); slot_idx++)
    {
      ...
      if (ImGui::IsItemHovered(ImGuiHoveredFlags_AllowWhenBlockedByActiveItem))
      {
        ...
        //Left click
        if(ImGui::IsMouseClicked(0) && (!ImGui::IsKeyDown(GLFW_KEY_LEFT_ALT)))
        {
